import { ApiResponse, SharePointFieldMetadata } from '../../types';
import { getWebUrl, normalizeFieldType, isGuid } from './utils';

export const createGetItemMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean,
  getFieldNames?: () => string[]
) => {
  return async (listName: string, itemId: number, targetListUrl?: string): Promise<ApiResponse<any>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createAddItemMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (listName: string, data: any, targetListUrl?: string): Promise<ApiResponse<any>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createUpdateItemMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (listName: string, itemId: number, data: any, targetListUrl?: string): Promise<ApiResponse<any>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createGetListItemsMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (listName: string, targetListUrl?: string): Promise<ApiResponse<any>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createUploadFileMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (listName: string, itemId: number, file: File, fileName?: string, targetListUrl?: string): Promise<ApiResponse<any>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createDeleteFileMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (listName: string, itemId: number, fileName: string, targetListUrl?: string): Promise<ApiResponse<any>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
      await targetWeb
...
export const createGetFieldMetadataMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (listName: string, fieldName: string, targetListUrl?: string): Promise<ApiResponse<SharePointFieldMetadata>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createGetListFieldsMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (listName: string, targetListUrl?: string): Promise<ApiResponse<SharePointFieldMetadata[]>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createSearchUsersMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (searchText: string, targetListUrl?: string): Promise<ApiResponse<any[]>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createGetUserByIdMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
) => {
  return async (userId: number, targetListUrl?: string): Promise<ApiResponse<any>> => {
    try {
      const targetUrl = getWebUrl(targetListUrl, baseUrl, isLocal, isInSharePoint);
      const targetWeb = targetUrl !== baseUrl ? new WebClass(targetUrl) : new WebClass(baseUrl);
...
export const createGetAttachmentFilesMethod = (
  WebClass: any,
  baseUrl: string,
  isLocal: boolean,
  isInSharePoint: boolean
